...
def addexpenses(request):
    ...
    if request.method=="POST":
        ...
        exp=Expenses.objects.create(family_lead=familylead,name=person_name,purpose=purpose,expense=expense,date=date)
...
def update_member(request,id):
    ...
    if request.method=="POST":
        family_mem.name=request.POST.get('name')
...
def updateexpenses(request, id):
    ...
    if request.method == "POST":
        ...
        expense.expense = float(request.POST.get('expense'))
        expense.date = request.POST.get('date')
        expense.save()
        return redirect('viewexpenses')

    return render(request, 'updateexpense.html', {'data': expense})
